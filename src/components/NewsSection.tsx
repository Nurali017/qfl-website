...
function FeaturedCard({ news, className }: { news: NewsItem; className?: string }) {
  return (
    <Link href={`/news/${news.id}`} className="group block h-full">
      <div className={`relative overflow-hidden rounded-2xl h-full bg-gray-100 transition-all duration-500 group-hover:shadow-xl group-hover:shadow-primary/10 ${className || 'min-h-[300px]'}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 p-4 sm:p-6 md:p-8 transition-transform duration-500 group-hover:-translate-y-1">
          ...
          <span className="text-white/90 text-sm drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] inline-block transition-all duration-500 group-hover:translate-x-1">{formatDate(news.publish_date, 'long')}</span>
        </div>
      </div>
    </Link>
  );
...
function HorizontalCard({ news, className }: { news: NewsItem; className?: string }) {
  return (
    <Link href={`/news/${news.id}`} className={`group block ${className || ''}`}>
      <div className="flex gap-4 sm:gap-3 p-4 bg-surface-soft hover:bg-white rounded-xl transition-all duration-300 hover:shadow-sm h-full">
        <div className="flex-1 min-w-0 flex flex-col">
          <h3 className="font-semibold text-gray-900 leading-snug group-hover:text-primary-light transition-colors">
            {news.title}
          </h3>
          <span className="text-gray-400 text-sm mt-auto pt-2 block transition-all duration-300 group-hover:translate-x-0.5">{formatDate(news.publish_date, 'long')}</span>
        </div>
...
function FeaturedSkeleton() {
  return (
    <div className="animate-pulse bg-gray-200 rounded-2xl h-full min-h-[300px]" />
  );
}

function HorizontalSkeleton({ className }: { className?: string }) {
  return (
    <div className={`flex gap-4 p-4 bg-surface-soft rounded-xl animate-pulse items-center ${className || ''}`}>
...
export function NewsFeatured() {
  ...
  return <FeaturedCard news={featuredNews} />;
}
  ...
  return (
    <div className="flex flex-col h-full gap-2">
      {loading ? (
        ...
        sideNews.map((news) => (
          <HorizontalCard key={news.id} news={news} className="flex-1" />
        ))
      )}
    </div>
  );
    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="lg:row-span-2">
          {loading ? <FeaturedSkeleton /> : featuredNews ? <FeaturedCard news={featuredNews} /> : null}
        </div>
