...
function HorizontalCard({ news, className, language = 'ru' }: { news: NewsItem; className?: string; language?: string }) {
  return (
    <Link href={`/news/${news.id}`} className={`group block ${className || ''}`}>
      <div className="flex gap-4 sm:gap-3 p-4 bg-surface-soft dark:bg-dark-surface-soft hover:bg-white dark:hover:bg-dark-surface-soft rounded-xl transition-all duration-300 hover:shadow-sm h-full">
...
function FeaturedSkeleton() {
  return (
    <div className="animate-pulse bg-gray-200 dark:bg-dark-surface-soft rounded-2xl h-full min-h-[300px]" />
  );
}
...
export function NewsFeatured() {
  ...
  const { latestNews, loading } = useLatestNews({ limit: 5, tournamentId: currentTournament.id });
...
export function NewsSideCards() {
  const { i18n } = useTranslation();
  const { currentTournament } = useTournament();
  const { latestNews, loading } = useLatestNews({ limit: 4, tournamentId: currentTournament.id });
