    ...
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-24 h-24 sm:w-[110px] sm:h-[110px]">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center pt-1">
          <span className="text-xl sm:text-2xl font-black text-gray-900 leading-none tracking-tighter" style={{ color }}>
            {value}<span className="text-sm font-bold text-gray-400">/{total}</span>
          </span>
        </div>
      </div>
      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest text-center mt-1">
        {sublabel}
      </span>
    </div>
  );
...
export function H2HOverallRecord({
  ...
  return (
    <div className="bg-[#f5f5f5] rounded-2xl md:rounded-[2rem] border border-gray-200/60 p-4 sm:p-6 md:p-10 shadow-lg max-w-4xl mx-auto">
      {/* Header */}
      ...
      {/* Progress Rings */}
      <div className="bg-white rounded-2xl p-4 sm:p-6 md:p-8 shadow-sm border border-gray-100 flex items-center justify-center gap-6 sm:gap-8 md:gap-12 w-full">
        <ProgressRing
